...
By CSS class names `.userContentWrapper` and `.ego_unit`.
...
This extension stores five persistent variables. All read and write access to the persistent variables must go through the object `DataStorage`. There is only one instance of `DataStorage` running in the extension, inside of `DataServer`.

The five persistant variables are:
- `disableMonitor` is a boolean that specifies whether the scanner is on or off. Default value is `disabled`.
- `disallowSurveys` is a boolean that specifies whether we are allowed to send surveys to the users. Default value is `disallow`. 
- `startDate` records the time when the scanner is turned on. Default value is `undefined` which will bring up a welcome or "cold start" message.
...
The extension contains two tab-separated files, extracted from public datasets released by ProPublica and New York University.

Both files contain strings and numbers only such as `targetType`, `targetValue`, advertisers' names, their total ads, total  impresssions, total ad spending on Facebook, etc.
...
This extension uses the public data sets collected by [ProPublica](http://projects.propublica.org/facebook-ads/) and [New York University](https://online-pol-ads.github.io/).
